...
- BLE 权限、扫描和基础连接框架，配对页可扫描附近 BLE 设备并建立 BLE 连接占位。
- 车辆状态读取未接入前默认显示未连接车辆，不展示 Mock 的已连接车辆状态。
...
3. 已完成 GPS 车速 Provider、驾驶模式检测和 debug APK 验收。
4. 已完成 BLE 权限、扫描、基础连接框架和配对页状态机占位。
5. 下一阶段优先做 Tesla 配对协议 PoC、P-256 密钥和安全存储。
6. 后续再做车辆状态读取、基础控制命令和真车内测。
